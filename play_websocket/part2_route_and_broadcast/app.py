...
async def play(websocket, game, player, connected):
    ...
    async for message in websocket:
        ...
        websockets.broadcast(connected, json.dumps(event))  # broadcast 方法不是协程
...
async def main():
    ...
    # handler 是管理连接的协程，当建立连接时websocket调用handler并将这个连接传递给handler，当handler结束时，websockets关闭这个连接。
