...
from PyQt5.QtWidgets import QWidget, QApplication, QHBoxLayout, QVBoxLayout, QPushButton, QLabel, QDialog
...
class Canvas(QLabel):
    def __init__(self, *args, parent=None, file_path=None):
        ...
        self.original_pix.loadFromData(el_image_bytes)
        self.setPixmap(self.original_pix)
...
class Stack(QDialog):
    ...
    def __init__(self, *args, parent=None):
        ...
        with open(file_path, 'rb') as f:
            el_image_bytes = f.read()
        original_pix.loadFromData(el_image_bytes)
        self.canvas.setPixmap(original_pix.scaled(300, 300))
        # - -
    ...
    def load_image(self):
        try:
            load_img_th = LoadImgThread(parent=self)
            load_img_th.update_image_signal.connect(self.update_canvas)
            load_img_th.err_signal.connect(self.thread_err_handler)
            load_img_th.start()
        except Exception as e:
            logger.error(f"e: {e}", exc_info=True)
    ...
    def update_canvas(self, data):
        try:
            ...
            self.layout_container.addWidget(b)
...
class LoadImgThread(QThread):
    ...
    def run(self):
        try:
            original_pix = QPixmap('../images/VI_TP3.jpg')
            self.label.setPixmap(original_pix)

            self.update_image_signal.emit({"canvas": self.label})
        except Exception as e:
            logger.errr(f"e: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    # label = QLabel()
    # original_pix = QPixmap()
    # file_path = '../images/VI_TP3.jpg'
    # with open(file_path, 'rb') as f:
    #     el_image_bytes = f.read()
    # original_pix.loadFromData(el_image_bytes)
    # label.setPixmap(original_pix.scaled(300,300))
    # label.show()
