...
class BaseTableWidget(QTableWidget):
    ...
    def insert_one_row(self, row_data, row: int):
        """
    ...
    def insert_rows(self, rows_data: list, refresh_all=False, specify_row: int = False):
        ...
        try:
            ...
            for i, row_data in enumerate(rows_data):
                row = start_row + i
                self.insert_one_row(row_data, row)
...
if __name__ == '__main__':
    ...
    class MyCheckboxTable(BaseTableWidget):
        ...
        def init_data(self, serial_num_offset=1):
        ...
        def insert_one_row(self, row_data: dict, row: int):
            """自己实现"""
    ...
    class ExampleWidget(QWidget):
        ...
        def init_data(self):
            self.check_box_table.init_data(serial_num_offset=1)
    ...
    my_widget.check_box_table.insert_rows(data, refresh_all=True)  # 删除历史数据，整表刷新
    # my_widget.check_box_table.insert_rows(data, refresh_all=False)  # 尾部添加数据加数据
