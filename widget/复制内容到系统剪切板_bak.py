...
class CopyBtn(QPushButton):
    ...
    def __init__(self, *args, parent=None, tip_text=None, empty_tip_text=None, empty_flag="",
                 tip_display_time=1.5, offset=10, tip_direction: int = 2):
        """
        :param args:
        :param parent: 父级实例
        :param tip_text: 提示信息
        :param tip_display_time: 提示信息显示时间（s）
        :param offset: 提示信息偏移量
    ...
    def set_tip_offset(self, offset):
    ...
    def calculate_tip_pos(self) -> QPoint:
        ...
        if self.tip_direction == TipDirection.left.value:
            tip_pos = QPoint(global_pos.x() - self.offset - self.tip_widget.width(),
            ...
            tip_pos = QPoint(global_pos.x() + (self.width() - self.tip_widget.width()) / 2,
                             global_pos.y() - self.tip_widget.height() - self.offset)
        elif self.tip_direction == TipDirection.right.value:
            tip_pos = QPoint(global_pos.x() + self.offset + self.width(),
                             global_pos.y() + (self.height() - self.tip_widget.height()) / 2)
        else:
            tip_pos = QPoint(global_pos.x() + (self.width() - self.tip_widget.width()) / 2,
                             global_pos.y() + self.height() + self.offset)
        return tip_pos
...
if __name__ == '__main__':
    ...
    class MyWidget(QWidget):
        ...
        def __init__(self, *args):
            ...
            self.product_code_copy_btn = CopyBtn(tip_direction=2, tip_display_time=1.5, parent=self)
            ...
            self.btn1.clicked.connect(self.btn1_clicked_event)
        ...
        def btn1_clicked_event(self):
            print(f"{self.objectName()}: 测试用的站位按钮1")
