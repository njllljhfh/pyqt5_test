...
class ScalePlate(QWidget):
    ...
    def set_font_ls_origin(self, font_ls=None):
        """设置自定义的标尺文字"""
        # TODO(tip):如果需要重新设置自定义的标签位置，请在resize之前调用该方法
        self.font_ls_origin = [str(item) for item in font_ls]

    def calculate_scale_pos_shown(self):
        """计算要显示的标尺的位置"""
        try:
            if self.pos_ls_origin is not None:
                self.calculate_custom_scale_pos_shown()
            else:
                # TODO(tip): 根据scale_num_origin，per来计算
                self.calculate_adaptive_scale_pos_shown()

            # print(f"self.pos_ls_shown = {self.pos_ls_shown}")
            self.generate_font_ls_shown()
            # print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ")
        except Exception as e:
            raise e

    def calculate_custom_scale_pos_shown(self):
        """计算实际显示的标尺刻度pos(用户自定义的pos)"""
        if self.direction == Qt.Horizontal:
            # print(f"============================= 水平标尺 =============================")
            # print(f"水平：self.ratio = {self.ratio}")
            self.pos_ls_shown = [
                [
                    pos[0] * self.ratio,
                    0,
            ...
            # print(f"============================= 竖直标尺 =============================")
            # print(f"竖直：self.ratio = {self.ratio}")
            self.pos_ls_shown = [
                [
                    0,
                    pos[1] * self.ratio,
    ...
    def calculate_scale_num_shown(self):
        """自适应pos时，计算实际显示的标尺个数"""
        res = self.scale_num_origin / self.per
        # print(f"self.scale_num_origin = {self.scale_num_origin}")
        # print(f"self.per = {self.per}")
        # print(f"res = {res}")

        scale_num_shown: int = math.ceil(res)  # 向上取整
        return scale_num_shown

    def calculate_adaptive_scale_pos_shown(self):
        """计算实际显示的标尺刻度pos(自适应，不适用自定义的标尺pos)"""
        self.pos_ls_origin = None

        self.scale_num_shown = self.calculate_scale_num_shown()
        # print(f"self.scale_num_shown = {self.scale_num_shown}")
        ...
        # 单位跨度
        span_unit = span_total / ((self.scale_num_shown - 1) * big_factor + small_factor)
        ...
        if self.direction == Qt.Horizontal:
            self.pos_ls_shown = [
                ...
                for i in range(self.scale_num_shown - 1)
            ]
    ...
    def resizeEvent(self, event: QResizeEvent):
        ...
        try:
            self.calculate_scale_pos_shown()
        except Exception as e:
            raise e
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        try:
            ...
            for index, scale_pos in enumerate(self.pos_ls_shown):
                ...
                painter.setPen(pen)
                font = str(self.font_ls_shown[index])
                scale_font = QFont()
...
if __name__ == '__main__':
    ...
    class MyWidget(QWidget):
        ...
        def init_data(self):
            ...
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # # pos自适应,文字默认
            # self.scale_plate_h_top.set_scale_num_origin(scale_num_origin=10)
            # self.scale_plate_h_bottom.set_scale_num_origin(scale_num_origin=10)
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # pos自适应,per自定义,文字自定义
            scale_num_origin = 80
            per = 1
            self.scale_plate_h_top.set_scale_num_origin(scale_num_origin=scale_num_origin)
            self.scale_plate_h_top.set_per(per=per)
            shown_num = self.scale_plate_h_top.calculate_scale_num_shown()
            font_ls_v = [f"{i+1}" for i in range(shown_num)]
            self.scale_plate_h_top.set_font_ls_origin(font_ls=font_ls_v)
            # - - -
            self.scale_plate_h_bottom.set_scale_num_origin(scale_num_origin=scale_num_origin)
            self.scale_plate_h_bottom.set_per(per=per)
            shown_num = self.scale_plate_h_bottom.calculate_scale_num_shown()
            font_ls_v = [f"{i+1}" for i in range(shown_num)]
            self.scale_plate_h_bottom.set_font_ls_origin(font_ls=font_ls_v)
            ...
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # pos自适应,文字默认
            # self.scale_plate_v_left.set_scale_num_origin(scale_num_origin=10)
            # self.scale_plate_v_right.set_scale_num_origin(scale_num_origin=10)
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # pos自适应,per=1,文字自定义
            scale_num_origin = 7
            font_ls_v = [chr(65 + i) for i in range(scale_num_origin)]
            self.scale_plate_v_left.set_scale_num_origin(scale_num_origin=scale_num_origin)
            self.scale_plate_v_left.set_font_ls_origin(font_ls=font_ls_v)
            self.scale_plate_v_right.set_scale_num_origin(scale_num_origin=scale_num_origin)
            self.scale_plate_v_right.set_font_ls_origin(font_ls=font_ls_v)
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # pos自适应,per自定义,文字自定义
            # scale_num_origin = 11
            # per = 4
            # self.scale_plate_v_left.set_scale_num_origin(scale_num_origin=scale_num_origin)
            # self.scale_plate_v_left.set_per(per=per)
            # shown_num = self.scale_plate_v_left.calculate_scale_num_shown()
            # font_ls_v = [chr(65 + i) for i in range(shown_num)]
            # self.scale_plate_v_left.set_font_ls_origin(font_ls=font_ls_v)
            # # - - -
            # scale_num_origin = 9
            # per = 3
            # self.scale_plate_v_right.set_scale_num_origin(scale_num_origin=scale_num_origin)
            # self.scale_plate_v_right.set_per(per=per)
            # shown_num = self.scale_plate_v_right.calculate_scale_num_shown()
            # font_ls_v = [chr(65 + i) for i in range(shown_num)]
            # self.scale_plate_v_right.set_font_ls_origin(font_ls=font_ls_v)
            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        def load_image(self, image_bytes=None):
            ...
            try:
                ...
                if not self.original_pix.isNull():
                    ...
                    scaled_pixmap = self.original_pix.scaled(QSize(*new_w_h), Qt.IgnoreAspectRatio,
