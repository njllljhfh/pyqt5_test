...
from PyQt5.QtCore import QUrl, QFileInfo, pyqtSignal, pyqtSlot, QObject, Qt
from PyQt5.QtWebChannel import QWebChannel
from PyQt5.QtWebEngineWidgets import QWebEngineView
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLineEdit, QSpacerItem, QSizePolicy
from xlsxwriter.exceptions import InvalidWorksheetName, FileCreateError
from api.dict_info_manager import DictInfoCRUD
from api.statistics.packed.defect_task import defectTask
from constants import DetectSite
from general.commonhelper import CommonHelper
from general.message import WarningMessageBox, InformationMessageBoxOne, CriticalMessageBoxOne
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.icon_button import IconButton
from general.mylog import logger
import numpy as np
from widgets.statistics.components.progress import ProgressBarWindow
from widgets.statistics.components.utils import gen_xlsx_path
...
class DefectTaskStatistics(QWidget):
    ...
    def slot_download(self):
        ...
        sheets.append(sheet)
        download_th = AsyncDownload(file_path, sheets, self)  # 异步下载excel表格
...
import os
from datetime import datetime
from PyQt5.QtChart import QChartView, QBarSet, QBarSeries, QAbstractBarSeries, QChart, QBarCategoryAxis, QValueAxis
from PyQt5.QtCore import Qt, pyqtSignal, QDate, QSize
from PyQt5.QtGui import QResizeEvent, QColor
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QTabWidget, QFrame, QSpacerItem, QSizePolicy, QHBoxLayout, QTableWidget, QAbstractItemView, QHeaderView, QGridLayout, QPushButton, QTableWidgetItem, QComboBox
from xlsxwriter.exceptions import FileCreateError, InvalidWorksheetName
from api.dict_info_manager import DictInfoCRUD
from api.statistics.packed.defect_category import defectCategory
from general.commonhelper import CommonHelper
from general.message import InformationMessageBoxOne, CriticalMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload
from widgets.statistics.components.combobox_group import ComboBoxGroup
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.icon_button import IconButton
from widgets.statistics.components.loading import LoadingWidget
from widgets.statistics.components.progress import ProgressBarWindow
from widgets.statistics.components.utils import gen_xlsx_path
from widgets.statistics.packed.defects_category_small import DefectCategorySmallStatistics
class DefectCategoryStatistics(QWidget):
    def __init__(self, main_window, *args, **kwargs):
        ...
        self.progress_bar = ProgressBarWindow(value_show=False)  # 下载进度条
    ...
    def slot_defect_class_small_btn_clicked(self, s_class_models):
        try:
            ...
            self.parent().DefectCategorySmallStatistics.ui.tab.setCurrentIndex(0)  # 默认显示列表界面
...
class DefectCategoryTable(QTableWidget):
    ...
    def update_data(self, rows_data):
        ...
        for i, model in enumerate(rows_data):
            row_data = [
                i + 1,  # 索引
                model.product_type,  # 产品类型
                model.pro_line_name,
                model.pro_spec,
                model.detect_site,
                model.defect_type,  # 缺陷类型
                model.detect_type,  # 检测类型
                model.defect_small_class_num,  # 缺陷小类数量
                model.repaired_ratio,  # 维修率
                model.scraped_ratio,  # 报废率
                model.defects_sum_num,  # 缺陷总数量
                model.defects_percent  # 分布占比
            ]
...
class DimissionFilter(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.config = []  # 生产线和规格配置文件，为异步请求获得
    ...
    def init_ui(self):
        ...
        self.pro_lines_combox = QComboBox()  # 生产线
        self.pro_specs_combox = QComboBox()  # 产品型号
        self.detect_sites_combox = QComboBox()  # 检测点
...
""" 缺陷类型分布统计 """
from PyQt5.QtChart import QBarSet, QBarSeries, QAbstractBarSeries, QBarCategoryAxis, QChart, QValueAxis, QChartView
from PyQt5.QtCore import Qt, pyqtSignal, QSize, QRect
from PyQt5.QtGui import QColor, QResizeEvent, QPixmap, QPaintEvent, QPainter
from PyQt5.QtWebEngineWidgets import QWebEngineView
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QTabWidget, QFrame, QSpacerItem, QSizePolicy, QHBoxLayout, \
    QTableWidget, QAbstractItemView, QHeaderView, QTableWidgetItem, QPushButton

from api.dict_info_manager import DictInfoCRUD
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne
from general.mylog import logger
from pyecharts import options
from pyecharts.charts import Pie
from pyecharts.commons.utils import JsCode
from widgets.statistics.components.graphBar import BarGraph
...
class DefectCategorySmallStatistics(QWidget):
    ...
    def load_data(self, data):
        ...
        数据格式：
            {
                缺陷小类: [0,1,2,0,0,0,0,0],    # 每个缺陷小类对应的每个缺陷状态（0 未处理状态，1 已维修状态， 2 已报废状态）
            }
        """
        try:
            self.ui.statistics_table.update_data(data)
            defect_class_percent_list = []  # 缺陷类别占比
            repaired_percent_list = []  # 平均修复率
            class_name_list = []
    ...
    def preview_table_data(self, data):
        ...
        for i, model in enumerate(data):
            results.append([
                ...
                model.detect_site,
                model.defect_class_name,  # 缺陷小类名
                model.welding,  # 串焊机
                model.defects_num,  # 缺陷数量
                model.defects_percent,  # 缺陷占比
                model.product_num,  # 产品数量
                model.average_defects,  # 平均缺陷数量
                model.average_repaired_ratio,  # 当前缺陷平均修复率
                model.scraped_ratio  # 报废率
            ])
        return results
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        try:
            ...
            if self.pixmap.size() != QSize(0, 0):
                x_ratio = (self.width() - 80) / self.pixmap.width()  # 留margin
...
from PyQt5.QtCore import QDate, pyqtSignal, QSize, Qt, QUrl, QFileInfo, QObject
from PyQt5.QtGui import QResizeEvent
from PyQt5.QtWebChannel import QWebChannel
from PyQt5.QtWebEngineWidgets import QWebEngineView
from PyQt5.QtWidgets import QWidget, QTableWidget, QTabWidget, QVBoxLayout, QLabel, QFrame, QHBoxLayout, \
    QSpacerItem, QSizePolicy, QHeaderView, QAbstractItemView, QGridLayout, QTableWidgetItem, QPushButton
from xlsxwriter.exceptions import FileCreateError, InvalidWorksheetName
import numpy as np

from api.statistics.packed.defect_pos import defectPos
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne, InformationMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload, AsyncDownloadPackedDetectPos
from widgets.statistics.components.combobox_group import ComboBoxGroup
from widgets.statistics.components.utils import gen_xlsx_path
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.icon_button import IconButton
...
class DefectPosStatistics(QWidget):
    def __init__(self, main_window, *args, **kwargs):
        ...
        # 定义控件
        self.cb_detect_type = self.ui.filters_pannel.cb_detect_type.comboBox  # 检测类型
        self.cb_line = self.ui.filters_pannel.cb_line.comboBox  # 生产线
        self.start_date = self.ui.filters_pannel.date_group.start_date  # 日起开始时间
        self.end_date = self.ui.filters_pannel.date_group.end_date  # 日期结束时间
        self.cb_date_selections = self.ui.filters_pannel.date_group.comboBox  # 日期快捷选择下拉框
        self.cb_defect_big_class = self.ui.filters_pannel.cb_defect_big_class.comboBox  # 缺陷大类
        self.cb_defect_type = self.ui.filters_pannel.cb_defect_type.comboBox  # 缺陷类型
        self.cb_statistics_dimension = self.ui.filters_pannel.cb_statistics_dimension.comboBox  # 统计维度
        self.cb_product_size = self.ui.filters_pannel.cb_product_size.comboBox  # 产品尺寸
        self.btn_search = self.ui.filters_pannel.btn_search  # 搜索按钮
        self.btn_download = self.ui.filters_pannel.btn_download  # 下载按钮
        self.progress_bar = ProgressBarWindow(value_show=False)  # 下载进度条
        # 鉴权
        self.btn_download.setEnabled(self.main_window.get_system_func("defect_statistic_download"))

        # 定义属性
        self.current_statistics_dimension_index = 0  # 记录当前查询条件的统计维度：0 产品尺寸；1 产品型号；点击搜索时更新此值
        self.is_loading = False  # 是否在加载数据，用于判断状态，限制界面操作
        self.loading_widget = LoadingWidget(":/icons/loading.gif", parent=self)
        self.current_product_spec = None  # 定义数组 (row, col)，用于记录当前查询的产品尺寸
        self.data_dict = None  # 用于存储查询数据的 map 字典
        self.data_models = None  # 用于存储转换成 model 格式的数据
        self.defect_big_class_type = None  # 缺陷大类名称
        self.defect_type = None  # 缓存缺陷类型

        # 定义下载excel的表头信息
        self.start_time_for_excel = ""  # 开始时间
        self.end_time_for_excel = ""  # 结束时间
        self.pro_line_for_excel = ""  # 生产线
        self.defect_type_for_excel = ""  # 缺陷类型
        self.excel_data = None

        # 初始化数据
        self.req_product_size()  # 要查询产品尺寸
        self.req_defect_types()  # 要查询缺陷类型
        self.req_product_line()  # 获取产线列表
    ...
    def slot_statistics_dimension_changed(self, index):
        ...
        try:
            if index == 1:  # 产品型号
                if self.cb_product_size.currentIndex() == 0:  # 默认产品尺寸选中第一个
                    # 查看是否有别的尺寸，没有不能选择改项
    ...
    def slot_product_size_changed(self, index):
        """ 切换产品尺寸 """
        try:
            if index == 0:  # 如果选择全部尺寸，则不能为产品型号统计维度
                if self.cb_statistics_dimension.currentIndex() == 1:  # 产品型号统计维度
    ...
    def req_defect_types(self):
        ...
        """
        try:
            self.cb_defect_big_class.setDisabled(True)  # 异步请求开始时禁用缺陷大类、缺陷类型下拉框
            self.cb_defect_type.setDisabled(True)
            self.defect_type_th = AsynThread(self.cruds.retrieve_defect_types, parent=self)
            self.defect_type_th.signal_result.connect(self.slot_update_defect_types)
            self.defect_type_th.start()
    ...
    def slot_update_defect_types(self, models):
        ...
        try:
            ...
            self.defect_type = {"全部": all_defect_types, "EL": el_defect_types, "VI": vi_defect_types}  # 组装缺陷类型对应el、vi数据
            ...
            self.cb_defect_big_class.setDisabled(False)  # 异步请求结束启用缺陷大类、缺陷类型下拉框
            self.cb_defect_type.setDisabled(False)
    ...
    def slot_defect_class_small_btn_clicked(self, s_class_models):
        ...
        try:
            ...
            self.parent().DefectPosSmallStatistics.ui.tab.setCurrentIndex(0)  # 默认显示列表界面
    ...
    def slot_btn_search_clicked(self):
        ...
        try:
            ...
            self.ui.statistics_table.setRowCount(0)  # 设置行数
            self.loading_widget.start()  # 加载数据提示
            ...
            self.req_statistics_info()  # 根据过滤条件查询统计信息
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    def req_statistics_info(self):
        ...
        try:
            # 获取查询参数
            detect_type_index = self.cb_detect_type.currentIndex()  # 检测方式的选择项
            product_line = self.cb_line.itemData(self.cb_line.currentIndex())  # 生产线ID
            start_datetime = self.convert_qdate_to_datetime(self.start_date.date)  # QDate
            end_datetime = self.convert_qdate_to_datetime(self.end_date.date)  # QDate
            defect_big_class_index = self.cb_defect_big_class.currentIndex()  # 缺陷大类选择项
            defect_type_text = self.cb_defect_type.currentText()  # 缺陷类型文本
            statistics_dimension_index = self.cb_statistics_dimension.currentIndex()  # 统计维度选项
            product_size = self.cb_product_size.currentData()  # 产品尺寸

            self.defect_big_class_type = self.cb_defect_big_class.currentText()  # 记录当前搜索缺陷大类的名称，下载时候需要

            self.ui.q_heatmap.defects_combobox.clear()  # 设置热力图下拉框
            defect_type = copy.copy(self.defect_type.get(self.defect_big_class_type))
            del defect_type[0]  # 删除 "全部" 选项
    ...
    def chart_load_data(self, map_size_and_pos_failure):
        ...
        """
        # 组织位置图表数据格式：
        # {产品尺寸:[[(0,0,count),(1,1,count),(2,2,count)], "failure_percent"]}
        chart_data = {}
        for product_size, postions_failure_products in map_size_and_pos_failure.items():
            ...
            postions_data = []  # 位置 和 位置个数的数据(row_index, col_index, count)：[(1,2,12)]
            for pos in pos_set:
                if pos:  # 注意 pos 中可能有 None 值，不计入
    ...
    def heatmap_load_data(self, map_defects_positions):
        """ 热力图加载数据
        :param map_defects_positions: 所有缺陷的缺陷位置 # {产品尺寸:{"隐裂": [(0,0),(0,1),(2,1)]}}
        20200617 需求：需要选择每一个【缺陷类】后，才可以显示对应的缺陷类热力图
        """
        try:
            self.ui.q_heatmap.set_map_defects_and_positions(map_defects_positions)  # 加载数据
            self.ui.q_heatmap.slot_change_filter_index(0)  # 触发重绘条件，这里传任何值，因为没用到
        except Exception as e:
            logger.error(e, exc_info=True)
...
class DefectPosTable(QTableWidget):
    ...
    def update_data(self, rows_data):
        ...
        for i, model in enumerate(rows_data):
            ...
            row_data = [
                i + 1,  # 索引
                temp_statistics_dimension,  # 产品尺寸 / 产品型号
                model.defects_sum_num,  # 缺陷总数量
                model.average_defects,  # 单产品平均缺陷数量
                model.defect_small_class_num,  # 缺陷类型数量（小类）
                model.most_defect_pos,  # 最高缺陷位
                model.failure_percent,  # 整体不良率
            ]
            self.data.append(row_data)
...
class QHeatMap(QFrame):
    def __init__(self, *args, **kwargs):
        ...
        self.init_ui()
        # 所有缺陷的缺陷位置: {产品尺寸:{"隐裂": [(0,0),(0,1),(2,1)]}}
        self.map_defects_and_pos = {}

        self.init_heatmap_page()  # 加载热力图页面
    ...
    def set_map_defects_and_positions(self, map_defects_and_positions):
        ...
        # 设置之后，要触发绘图
        self.slot_change_filter_index(0)  # 参数无用
    ...
    def slot_change_filter_index(self, index):
        ...
        try:
            ...
            # 注意：切换的 defect 可能没有对应的缺陷，没有该缺陷数据，要置为空
    ...
    def draw_heatmap(self, pos_list):
        ...
        for row in range(rows_num):
            for col in range(cols_num):
                data.append([col, len(rows) - 1 - row, pos_list.count((row, col))])  # (x,y,num)

        heatmap_data = []
        min_value = 0
        max_value = 0
        for d in data:
            heatmap_data.append([d[0], d[1], d[2] or '-'])  # (col,row, value)
            if d[2] > max_value:
                max_value = d[2]
            if d[2] < min_value:
                min_value = d[2]
...
class DefectRateTableChart(QTableWidget):
    ...
    def set_product_spec(self, row, col):
        ...
        self.set_column_width(len(col_labels) - 1, 100)  # 设置最后一列固定宽度
        ...
        try:
            for pos_data in positions_data:
                ...
                if col_index + 1 > self.spec_col or row_index + 1 > self.spec_row:
                    continue
                if col_index < 0:  # col_index会有小于0的情况，忽略
                    continue
                col_index += 1
...
class Shared(QObject):
    ...
    连接web页面管道类
    """
    signal_data_success = pyqtSignal(str)  # 查询任务下缺陷类列表成功信号
    signal_data_error = pyqtSignal(str)  # 查询任务下缺陷类列表失败信号
...
from PyQt5.QtGui import QResizeEvent
from PyQt5.QtWidgets import QWidget, QTableWidget, QTabWidget, QVBoxLayout, QLabel, QFrame, QHBoxLayout, \
    QSpacerItem, QSizePolicy, QHeaderView, QAbstractItemView, QTableWidgetItem, QPushButton
...
class DefectPosSmallTable(QTableWidget):
    ...
    def preview_table_data(self, data):
        ...
        for i, model in enumerate(data):
            results.append([
                i + 1,  # 索引
                model.temp_statistics_dimension,  # 统计维度；因为显示的时候要从【产品型号】/【产品尺寸】中选择一个，所以作为临时显示数据用
                model.defect_class_name,  # 缺陷小类名
                model.defects_num,  # 缺陷数量
                model.defects_percent,  # 缺陷占比
                model.product_num,  # 产品数量
                model.most_defect_pos,  # 最高缺陷位
                model.failure_percent,  # 不良率
            ])
        return results
...
from PyQt5.QtGui import QResizeEvent, QColor
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QTabWidget, QTableWidgetItem, QHeaderView, QAbstractItemView, \
    QTableWidget, QFrame, QGridLayout, QSpacerItem, QSizePolicy, QLabel, QHBoxLayout

from api.statistics.packed.error_and_omission import errorAndOmission
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne, InformationMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownloadPackedErrorOmission
from widgets.statistics.components.combobox_group import ComboBoxGroup
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.graphBar import BarGraph
from widgets.statistics.components.icon_button import IconButton
...
class ErrorOmissionStatistics(QWidget):
    ...
    def req_statistics_info(self):
        ...
        try:
            # 获取查询参数
            start_datetime = self.convert_qdate_to_datetime(self.start_date.date)  # QDateTime
            end_datetime = self.convert_qdate_to_datetime(self.end_date.date)  # QDateTime
            statistics_dimension_index = self.cb_statistics_dimension.currentIndex()  # 统计维度选项
    ...
    def slot_get_statistics_info(self, models):
        ...
        try:
            ...
            self.data_rows = []  # 清空数据，避免下载错误的
            logger.error(e, exc_info=True)
            CriticalMessageBoxOne("处理查询的统计数据出错")
        finally:
            self.loading_widget.stop()  # 停止加载等待效果
...
from PyQt5.QtChart import QBarSeries, QChartView, QBarSet, QChart, QBarCategoryAxis, QValueAxis, QAbstractBarSeries
from PyQt5.QtCore import Qt, pyqtSlot, pyqtSignal, QThread
from PyQt5.QtGui import QIcon, QPainter, QColor, QResizeEvent
from PyQt5.QtWidgets import QVBoxLayout, QWidget, QHBoxLayout, QLabel, QComboBox, QTabWidget, \
    QSpacerItem, QGridLayout, QAbstractItemView, QTableWidget, QHeaderView, QSizePolicy, QTableWidgetItem, QFileDialog
from xlsxwriter.exceptions import FileCreateError, InvalidWorksheetName

from api.statistics.packed.defect_and_scrap import defectAndScrap
from constants import ProductStatisticDimension, ProductInferenceMode, ProType
from general.commonhelper import CommonHelper
from general.message import WarningMessageBoxOne, InformationMessageBoxOne, CriticalMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsyncDownload
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.icon_button import IconButton
from widgets.statistics.components.loading import LoadingWidget
from widgets.statistics.components.progress import ProgressBarWindow
...
class UI_ProductStatistic(object):
    ...
    def __init__(self, parent):
        ...
        # 2. 产品统计QTabWidget
        self.product_statistic_tab = ProductStatisticTab(parent=parent)  # 列表tab/图表tab
...
class ProductStatistic(QWidget):
    ...
    def __init__(self, main_window, *args, parent=None, **kwargs):
        ...
        self.qss_style = CommonHelper.readQss(style_file)  # 全局样式

        self.main_window = main_window
        self.product_statistic_thread = ProductStatisticThread()  # 产品统计分析线程
        self.loading_widget = LoadingWidget(":/icons/loading.gif", parent=self)

        if self.main_window.get_system_func("defect_baofei_statistic_download") is False:
            self.ui.download_btn.setEnabled(False)

        self.setStyleSheet(self.qss_style)  # 设置全局样式
        ...
        self.product_statistic_thread.err_msg_signal.connect(self.thread_err_msg_signal_handler)  # 处理线程异常
        self.product_statistic_thread.statistic_result_signal.connect(self.update_statistic_data_handler)  # 更新统计数据
...
class ProductStatisticTab(QTabWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 1.表格
        self.table_tab = QWidget()  # tab1:产品统计分析-表格
        ...
        # 2.图表
        self.chart_tab = QWidget()  # tab2:产品统计分析-图表
...
class ProductStatisticTable(QTableWidget):
    ...
    def __init__(self, *args, parent=None):
        ...
        self.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 取消水平滚动条
        ...
        # 设置列宽
        self.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)  # 其他列列宽均分
    ...
    def insert_rows(self, rows_data, refresh_all=True):
        ...
        try:
            ...
            existed_rows = self.rowCount()  # 获取已有数据行数
    ...
    def _set_column_width(self):
        """设置列宽度"""
        if self.rowCount() > 0:
            self.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)  # 自适应并固定
...
class ProductStatisticChart(QChartView):
    ...
    def __init__(self, parent=None, default_bar_ls: [str, ...] = None):
        ...
        self.series.setLabelsPosition(QAbstractBarSeries.LabelsCenter)  # 设置数据系列标签的位置于数据柱内测上方
        self.series.setLabelsVisible(True)  # 在柱形图上显示数据的值
        self.series.setLabelsPosition(QAbstractBarSeries.LabelsInsideBase)
        self.chart = QChart()
        self.chart.addSeries(self.series)
        self.chart.setTitle("产品缺陷率和报废率分布（%）")
        self.chart.setAnimationOptions(QChart.SeriesAnimations)  # 动画效果
    ...
    def set_axis_x(self, categories: list):
        ...
        :return:
        """
        # 如果categories中有重复的字段，该append()方法会进行去重
        if categories:
    ...
    def set_name_and_value_of_bars(self, bar_set_date: dict):
        ...
        :param bar_set_date:
            举例：
                {
                    "报废率": [10, 20, 30], # 不同的x对应的值y值
                    "缺陷率": [40, 50, 60], # 不同的x对应的值y值
                }
        :return:
        """
        try:
...
class ProductStatisticThread(QThread):
    """异步查询产品数据并计算统计数据-线程"""
    err_msg_signal = pyqtSignal(str)  # 失败,错误信息
    statistic_result_signal = pyqtSignal(dict)  # 异步查询产品数据并计算统计数据,成功
...
from api.dict_info_manager import DictInfoCRUD
from api.statistics.unpacked.defect_task import defectTask
from constants import DetectSite
from general.commonhelper import CommonHelper
from general.message import WarningMessageBox, InformationMessageBoxOne, CriticalMessageBoxOne
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload
...
class Shared(QObject):
    ...
    连接web页面管道类
    """
    signal_query_begin = pyqtSignal()  # 开始查询信号
    signal_success = pyqtSignal(str)  # 查询任务列表成功信号
    signal_error = pyqtSignal(str)  # 查询任务列表失败信号
    signal_data_success = pyqtSignal(str)  # 查询任务下缺陷类列表成功信号
    signal_data_error = pyqtSignal(str)  # 查询任务下缺陷类列表失败信号
...
class DefectTaskStatistics(QWidget):
    def __init__(self, main_window, *args, **kwargs):
        ...
        self.search_input = None  # 搜索框控件
        self.start_time_input = None  # 开始时间日历控件
        self.end_time_input = None  # 结束时间日历控件
        self.search_btn = None  # 搜索按钮控件
        self.download_btn = None  # 下载按钮控件
        self.browser = None  # 浏览器控件
        self.channel = None  # 浏览器web页面管道

        self.init_ui()  # 初始化样式
        self.init_event()  # 初始化事件信号
        self.load_web_page()  # 加载web页面
        self.init_channel()  # 初始化web页面信号
    ...
    def init_event(self):
        ...
        :return:
        """
        self.search_input.returnPressed.connect(self.req_statistics_info)  # 搜索框回车事件
        self.search_btn.clicked.connect(self.req_statistics_info)  # 搜索按钮点击事件
        self.download_btn.clicked.connect(self.slot_download)  # 下载按钮点击事件
    ...
    def slot_download(self):
        ...
        self.progress_bar.set_title("正在下载 %s" % file_name)  # 进度条
        self.progress_bar.show()
    ...
    def gen_download_data(self):
        ...
        try:
            gg_enum = DictInfoCRUD.get_all_gg_enum(True)  # 获取规格数据枚举
...
from PyQt5.QtGui import QResizeEvent, QColor
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QTabWidget, QFrame, QSpacerItem, QSizePolicy, QHBoxLayout, \
    QTableWidget, QAbstractItemView, QHeaderView, QGridLayout, QPushButton, QTableWidgetItem, QComboBox
from xlsxwriter.exceptions import FileCreateError, InvalidWorksheetName

from api.dict_info_manager import DictInfoCRUD
from api.statistics.unpacked.defect_category import defectCategory
from general.commonhelper import CommonHelper
from general.message import InformationMessageBoxOne, CriticalMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload
...
class DefectCategoryStatistics(QWidget):
    def __init__(self, main_window, *args, **kwargs):
        ...
        self.start_time_for_excel = ""  # 开始时间
        self.end_time_for_excel = ""  # 结束时间
        self.defect_type_for_excel = ""  # 搜索维度

        # 初始化数据
        self.req_defect_types()  # 获取产品缺陷类型
        self.req_pro_lines_spec()
    ...
    def slot_btn_search_clicked(self):
        ...
        try:
            ...
            self.current_pro_line = self.cb_statistics_dimension.pro_lines_combox.currentText()  # 记录当前产线名称
            self.current_pro_spec = self.cb_statistics_dimension.pro_specs_combox.currentText()  # 记录当前产品规格
            self.welding = self.cb_statistics_dimension.welding_combox.currentText()  # 记录当前检测点

            self.start_time_for_excel = self.start_date.text()  # 记录当前开始时间给excel表格使用
            self.end_time_for_excel = self.end_date.text()  # 记录当前结束时间给excel表格使用
            self.defect_type_for_excel = self.cb_defect_big_class.currentText()  # 记录检测方式给excel表格使用
            self.ui.statistics_table.setRowCount(0)  # 清除列表中的数据
            self.loading_widget.start()  # 加载数据提示
            self.req_statistics_info()  # 根据过滤条件查询统计信息
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    def slot_get_statistics_info(self, models):
        ...
        try:
            ...
            self.req_pro_lines_spec()  # 动态更新统计维度信息
        except Exception as e:
            logger.error(e, exc_info=True)
        finally:
            self.loading_widget.stop()
    ...
    def download_excel(self, file_path):
        ...
        # 缺陷小类表格
        data_1_row_num = self.ui.statistics_table.rowCount()  # 用于查询有几个子类table
        sub_table_data = []
        for page_index in range(data_1_row_num):  # 循环生成统计小类表格
            # 找到每一行的子类数据
            _, data_s_table = self.get_small_class_table_data(page_index)
            sub_table_data.extend(data_s_table)
    ...
    def slot_defect_class_small_btn_clicked(self, s_class_models):
        ...
        try:
            ...
            self.parent().unpackedDefectCategorySmallStatistics.ui.tab.setCurrentIndex(0)  # 默认显示列表界面
            self.main_window.init_navigation_bar_fun(
...
class DimissionFilter(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.config = []  # 生产线和规格配置文件，为异步请求获得
        self.pro_line_id = ''  # 产线ID
        self.detect_site = ''  # 检测点
        self.spec = ''  # 规格

        self.pro_lines = ''  # 当前选择的产线名称
        self.pro_specs = ''  # 当前选择的产品规格
        self.detect_sites = ''  # 当前选择的检测点
    ...
    def init_ui(self):
        ...
        self.pro_lines_combox = QComboBox()  # 生产线
        self.pro_specs_combox = QComboBox()  # 产品型号
        self.welding_combox = QComboBox()  # 串焊机
...
from api.statistics.unpacked.defect_pos import defectPos
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne, InformationMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload, AsyncDownloadPackedDetectPos
...
class DefectPosStatistics(QWidget):
    def __init__(self, main_window, *args, **kwargs):
        ...
        self.cb_detect_type = self.ui.filters_pannel.cb_detect_type.comboBox  # 检测类型
        self.cb_line = self.ui.filters_pannel.cb_line.comboBox  # 生产线
        self.cb_weld = self.ui.filters_pannel.cb_weld.comboBox  # 焊接机
        self.start_date = self.ui.filters_pannel.date_group.start_date  # 日起开始时间
        self.end_date = self.ui.filters_pannel.date_group.end_date  # 日期结束时间
        self.cb_date_selections = self.ui.filters_pannel.date_group.comboBox  # 日期快捷选择下拉框
        self.cb_defect_big_class = self.ui.filters_pannel.cb_defect_big_class.comboBox  # 缺陷大类
        ...
        self.pro_lines_config = None  # 生产线和串焊机配置
    ...
    def slot_line_changed(self, index):
        ...
        try:
            ...
            if not pro_line_id:  # 生产线为全部时，需要列出所有的串焊机
    ...
    def req_statistics_info(self):
        ...
        try:
            # 获取查询参数
            detect_type_index = self.cb_detect_type.currentIndex()  # 检测方式的选择项
            product_line = self.cb_line.currentData()  # 生产线ID
            weld = self.cb_weld.currentData()  # 串焊机
            start_datetime = self.convert_qdate_to_datetime(self.start_date.date)  # QDate
            end_datetime = self.convert_qdate_to_datetime(self.end_date.date)  # QDate
            defect_big_class_index = self.cb_defect_big_class.currentIndex()  # 缺陷大类选择项
            defect_type_text = self.cb_defect_type.currentText()  # 缺陷类型文本
...
class FiltersPannel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.cb_line = ComboBoxGroup("生产线:", ["全部", "L2", "L3"])
        layout2.addWidget(self.cb_line, 0, 1)
        self.cb_weld = ComboBoxGroup("串焊机:", ["全部", "1号", "2号", "3号", "4号", "5号", "6号"])  # 串焊机
...
from api.statistics.unpacked.error_and_omission import errorAndOmission
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne, InformationMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownloadUnpackedErrorOmission
from widgets.statistics.components.combobox_group import ComboBoxGroup
from widgets.statistics.components.date_group import DateGroup
from widgets.statistics.components.graphBar import BarGraph
from widgets.statistics.components.icon_button import IconButton
...
from general.mylog import logger
# 轻量版打包需要将下面2行注释掉
from widgets.statistics.components.xlsx import XlsxWtiter
from widgets.statistics.components.excel.error_omission.packed_error_omission_excel import packedErrorOmissionExcel
from widgets.statistics.components.excel.error_omission.unpacked_error_omission_excel import unpackedErrorOmissionExcel
from widgets.statistics.components.excel.defect_pos.defect_pos_excel import DefectPosExcel
...
""" 带label标签的下拉列表组合控件 """
from PyQt5.QtWidgets import QFrame, QHBoxLayout, QLabel, QComboBox, QListView, QSpacerItem, QSizePolicy
...
from PyQt5.QtCore import QDate, pyqtSignal, Qt, QDateTime
from PyQt5.QtGui import QMouseEvent
from PyQt5.QtWidgets import QLineEdit, QCalendarWidget, QComboBox, QListView, QLabel, QHBoxLayout, QSizePolicy, \
...
class DateGroup(QFrame):
    def __init__(self, label, options=None, default=0, *args, **kwargs):
        ...
        self.__current_lineEdit__ = 0  # 当前点击的是哪个输入框
    ...
    def slot_date_range_changed(self, index):
        ...
        try:
            ...
            if index == 0:
                ...
                # fixme 20200629: 不能直接加6，超过本月天数，会返回空
...
class DateLineEdit1(QLineEdit):  # 日期显示的文本框
...
from PyQt5.QtGui import QPaintEvent, QColor, QPainter, QPen, QBrush, QLinearGradient, QFontMetrics
    ...
    def paintEvent(self, event: QPaintEvent):
        try:
            ...
            if self.data_frame and all(self.data_frame):
                ...
                # if self.bar_margin * (bar_num - 1) + self.bar_width * (bar_num + 2) > 1:
                #     self.bar_margin = 0
                ...
                for i, x_label in enumerate(self.x_axis_labels):
                    # 绘制bar背景，绘制 x 轴 labels，记住每个 bar 的起始位置
                    ...
                    for j in range(bar_num):
                        ...
                        painter.setBrush(brush)
                        bar_x = bar_start_x + (self.bar_width + self.bar_margin) * per_scale_w * j  # bar 的真实位置
...
from PyQt5.QtWidgets import (QHBoxLayout, QLabel, QSizePolicy, QVBoxLayout, QPushButton, QSpacerItem,
                             QFileDialog, QDialog)
from PyQt5.QtGui import QPixmap, QMovie, QIcon, QImage

from general.message import InformationMessageBoxOne, CriticalMessageBoxOne, InformationMessageBox
from widgets.statistics.components.progress import ProgressBarWindow
...
class ImgViewer(QDialog):
    def __init__(self):
        super().__init__()
        self.container = None  # 渲染图片容器
        self.loading_container = None  # loading图片容器
        self.hbox = None  # 状态栏布局layout
        self.loading = None  # loading动画
        self.data = None  # 数据详细数据
        self.progress_bar = None  # 下载进度条框

        self.file_path = None  # 文件路径

        self.th = None  # 异步获取图片线程
        self.initUI()
    ...
    def initUI(self):
        ...
        """
        self.setWindowIcon(QIcon(':/icons/title.png'))
        self.setStyleSheet("""background: white;""")  # 设置窗口背景色
    ...
    def render_img(self, data):
        ...
        try:
            ...
            if not img_ok:
                container.setScaledContents(False)  # 不允许QLabel缩放它的内容充满整个可用的空间
            else:
                container.setScaledContents(True)  # 允许QLabel缩放它的内容充满整个可用的空间

            self.resize(1300, int(1300 / ratio) + 60)

            self.setWindowTitle(win_title)  # 设置窗体标题
    ...
    def render_detail(self, img_ok):
        ...
        try:
            ...
            self.spacer = QSpacerItem(30, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)  # 弹簧
            ...
            self.spacer1 = QSpacerItem(30, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)  # 弹簧
            ...
            self.spacer2 = QSpacerItem(30, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)  # 弹簧
            ...
            self.spacer3 = QSpacerItem(30, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)  # 弹簧
            ...
            self.spacer4 = QSpacerItem(30, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)  # 弹簧
...
class DownloadThread(QThread):
    ...
    下载图片异步线程
    """
    signal_result = pyqtSignal(dict)  # 下载成功信号
    signal_processing = pyqtSignal(int)  # 下载进度信号
    signal_error = pyqtSignal()  # 下载错误信号
    ...
    def run(self):
        ...
        """
        try:
            file_size = os.path.getsize(self.file_path)  # 文件字节大小
            coped_size = 0
            ...
            with open(self.file_path, 'rb') as ofp, open(self.dst_path, 'wb') as nfp:
                while True:
                    ...
                    coped_size += len(content)  # 已经拷贝文件字节大小
                    percent = int(coped_size / file_size * 100)  # 复制百分比
...
class AsynThread(QThread):
    ...
    def __init__(self, file_path, error_file_path, not_found_file_path, img_max_width, parent=None):
        ...
        self.error_file_path = error_file_path  # 显示错误图片路径
        self.not_found_file_path = not_found_file_path  # 显示图片文件不存在路径
        self.img_max_width = img_max_width  # 图片最大宽度
    ...
    def resize(self):
        ...
        try:
            ...
            defect_img = os.path.join(dir_name, 'defect_' + file_name)  # 降采后的图片小，可以更快的显示

            if os.path.exists(defect_img):
                img = cv2.imread(defect_img, cv2.IMREAD_UNCHANGED)  # cv2读取图片
                if img is None:
                    img = cv2.imread(self.file_path, cv2.IMREAD_UNCHANGED)  # cv2读取图片
            else:
                img = cv2.imread(self.file_path, cv2.IMREAD_UNCHANGED)  # cv2读取图片
            ...
            orign_width, orign_height = img.shape[1], img.shape[0]  # 获取原始图片的宽高

            if orign_width < self.img_max_width:  # 图片小于最小宽度则不进行缩放
                scale_percent = 1
            else:
                scale_percent = self.img_max_width / orign_width

            width, height = int(orign_width * scale_percent), int(orign_height * scale_percent)  # 计算缩放后的图片宽高

            resized = cv2.resize(img, (width, height), interpolation=cv2.INTER_AREA)  # 缩放

            cvimg = cv2.cvtColor(resized, cv2.COLOR_BGR2RGB)  # 转换为QImage可使用的图片

            cvimg = QImage(cvimg.data, width, height, QImage.Format_RGB888)  # 生成QImage对象
    ...
    def run(self):
        ...
        try:
            ...
            if not os.path.exists(self.file_path):  # 图片文件不存在，显示不存在图片logo
                pixmap = QPixmap(self.not_found_file_path)
                img_ok = False
                file_name = "%s 图片不存在" % file_name
            else:
                resized_img = self.resize()  # 获取缩放后的图片
                if not resized_img:  # 图片已损坏
                    img_ok = False
                    pixmap = QPixmap(self.error_file_path)  # 无法打开图片，显示图片无法打开
...
#     """
#     file_dialog = QFileDialog()
#     path = file_dialog.getSaveFileName(caption=caption, directory=os.path.join(directory, file_name),
#                                        filter="Excel 工作簿(*.xlsx)")[0]
#
#     return path
# -*- coding:utf-8 -*-
# -*- author:menglingkuo -*-
...
class XlsxWtiter(object):
    def __init__(self):
        self.workbook = xlsxwriter.Workbook  # 对象，需要实例化

    def save_excel(self, sheets, path):
        """ 保存excel文件到本地
        :param title: 保存的EXCEL文件名，不带 .xlsx 后缀
        :param sheets: excel 的 worksheet 的数据，为列表
            [
                [
                    "sheet1_title",
                    {    # 每个cell的数据，key 为单元格的位置，如果merge的单元格则需要标明范围
                        (2, 1, 3, 3): {           # 数组格式合并单元格 (first_row, first_col, last_row, last_col)
                            value: "",      # 值
                            format: "format_name"     # 单元格格式
                            merge: True,          # 是否合并单元格
                        },
                        "A1:J1": {},              # 范围格式单元格合并
                        (2,1): {},                # 数组格式单个单元格
                    ...
                    {   # 单元格格式字典
                        "cell_format_1": {    # 单元格格式1, workbook.add_format 的字典参数
                            'align': 'center',  # 水平居中
                            'valign': 'vcenter',  # 垂直居中
                            'font_size': '12',  # 颜色填充
                            'fg_color': '#b4c6e7',  # 颜色填充
                        },
                        "cell_format_2": {    # 单元格格式2
                            'align': 'center',  # 水平居中
                            'valign': 'vcenter',  # 垂直居中
                            'font_size': '12',  # 颜色填充
                            'fg_color': '#b4c6e7',  # 颜色填充
        ...
        :param caption: 保存弹出对话框窗体名称
        :param directory: 打开窗体默认的文件夹目录，Windows下 '' 为桌面路径
        :return:
        """
        # 参数校验
        # 统计分析下载的 excel 表格默认命名方式：
        # 格式：[统计分析表]_[下载日期].xlsx
        # 举例：产品缺陷率和报废率统计表_202007014.xlsx
        if not path:  # 点击取消，为空字符串
            return
        ...
        try:
            for sheet in sheets:  # 根据传参数据组织 sheet 页面
                # 遍历每一个 worksheet
                ...
                for pos, cell in sheet_data.items():
                    ...
                    #
                    if len(pos) == 4:  # 说明是范围
                        if cell.get("merge"):  # 要合并单元格；不考虑自适应宽度
                            worksheet.merge_range(*pos, cell["value"], formats[cell["format"]])
                        else:
                            for r in range(pos[0], pos[2] + 1):  # 行
                                for c in range(pos[1], pos[3] + 1):
                                    worksheet.write(r, c, cell["value"], formats[cell["format"]])
                                    # 自适应宽度
                                    width = self.len_byte(str(cell["value"])) + 2  # 列宽按字节长度计算多加2
                                    existed_width = cols_width.get(c, 10.27)
                                    if width > existed_width:  # 不能小于默认宽度和已有宽度
                                        cols_width.update({c: width})  # 赋值
                                    else:
                                        cols_width.update({c: existed_width})  # 赋值
                        ...
                        if width > existed_width:  # 不能小于默认宽度和已有宽度
                            cols_width.update({pos[1]: width})  # 赋值
                        else:
                            cols_width.update({pos[1]: existed_width})  # 赋值

                # 自动调整列宽
                for col, width in cols_width.items():
                    worksheet.set_column(col, col, width)  # 设置宽度

                # 其他一些options配置项
                for row_height in sheet_options.get("height", []):
                    worksheet.set_row(*row_height)  # 设置行高
        except Exception:
            raise
        finally:
            workbook.close()
    ...
    def convert_range(*args):
        """ 将范围变成 first_row, first_col, last_row, last_col
        :param args:
    ...
    def __auto_scale_width__(self, worksheet, dataTable):
        ...
        # 要自适应宽度的列索引列表，不指定则自适应所有的列，默认列宽为 10.27
        ...
        for r, row in enumerate(dataTable):
            for c, cell in enumerate(row):
                # 判断是否有该列
                if c == len(cols_width):
                    cols_width.append(10.27)  # 给定默认宽度
                if cell is not None:  # 如果不是 None，则为正常值
                    # 列宽按字节长度计算多加2
                    width = self.len_byte(str(cell)) + 2
                    if width > 10.27 and width > cols_width[c]:  # 不能小于默认宽度和已有宽度
                        cols_width[c] = width  # 赋值
                        worksheet.set_column(c, c, width)  # 设置宽度
...
from PyQt5.QtGui import QResizeEvent, QColor
from PyQt5.QtWidgets import QWidget, QTabWidget, QVBoxLayout, QGridLayout, QFrame, QLabel, QHBoxLayout, QSpacerItem, \
    QSizePolicy, QTableWidget, QHeaderView, QAbstractItemView, QPushButton, QTableWidgetItem
from xlsxwriter.exceptions import FileCreateError, InvalidWorksheetName

from api.statistics.smart_defect import smartDefect
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne, InformationMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.asyn_thread import AsynThread, AsyncDownload
...
class TaskPerformanceStatistics(QWidget):
    ...
    def slot_btn_search_clicked(self):
        ...
        try:
            ...
            self.ui.statistics_table.setRowCount(0)  # 设置行数
            self.loading_widget.start()  # 加载数据提示
            self.current_statistics_dimension_index = self.cb_statistics_dimension.currentIndex()  # 修改当前搜索维度
    ...
    def slot_defect_class_small_btn_clicked(self, models):
        ...
        try:
            ...
            self.parent().TaskPerformanceSmallStatistics.ui.tab.setCurrentIndex(0)  # 默认显示列表界面
            self.main_window.init_navigation_bar_fun(
    ...
    def chart_load_data(self, charts):
        ...
        try:
            ...
            for i, chart in enumerate(charts):
                bar = BarGraph()
                bar.set_bar_bgcorlor(QColor(70, 70, 70))
                bar.set_bar_width(0.025 * pro_counts)  # 根据数量动态设置柱状图宽度
                bar.set_bar_margin(0.005 * pro_counts)  # 根据数量动态设置柱状图margin
                bar.set_data_frame(*chart[0], **chart[1])
                self.ui.q_charts.add_widget(bar)
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    def update_data(self, rows_data):
        ...
        for i, model in enumerate(rows_data):
            row_data = [
                ...
                model.obj_type,
                model.task_num,  # 任务数量
                model.detect_type,  # 将测方式
                model.failure_num,  # 故障次数
                model.failure_percent,  # 故障率
                model.average_detect_time,  # 平均检测时间
                model.max_detect_time,  # 最高检测时间
                model.min_detect_time,  # 最低检测时间
                model.detect_failure_times,  # 检测失败次数
                model.detect_failure_percent,  # 检测失败率
            ]
            self.data.append(row_data)
            ...
            for col, data in enumerate(row_data):
                if col == 6:
                    ...
                    q_btn.setCursor(Qt.PointingHandCursor)
                    # 提交点击的信号
                    # q_btn.clicked.connect(self.slot_failure_number_btn_clicked(model.small_class_models))
                    self.setCellWidget(i, col, q_btn)
                elif col == 11:
                    q_btn = QPushButton(str(data))
                    q_btn.setStyleSheet(btn_qss)
...
from PyQt5.QtWidgets import QWidget, QTableWidget, QTabWidget, QVBoxLayout, QLabel, QHBoxLayout, \
    QSpacerItem, QSizePolicy, QHeaderView, QAbstractItemView, QTableWidgetItem, QPushButton, QDialog

from api.config import CONSTANT
from general.commonhelper import CommonHelper
from general.message import CriticalMessageBoxOne
from general.mylog import logger
from widgets.statistics.components.img_viewer import ImgViewer
...
class FailureStatisticSmallModel(object):
    def __init__(self):
        ...
        self.img_name = ""  # 图像名称
        self.img_path = ""  # 图像路径
        self.start_time = ""  # 检测开始时间
        self.end_time = 0  # 检测结束时间
